...
if args.plink == None:
	...
	likeMatrix = pd.read_csv(str(args.beagle), sep="\t", engine="c", header=0, usecols=list(range(3, 3 + 3*args.n)), dtype=np.float32, compression="gzip")
	likeMatrix = likeMatrix.as_matrix().T
else:
	chunk_N = int(np.ceil(float(args.n)/args.threads))
	chunks = [i * chunk_N for i in range(args.threads)]
